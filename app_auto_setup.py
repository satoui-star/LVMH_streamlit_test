...
@st.cache_resource
def get_resources():
    """Charge ChromaDB et le modèle d'embeddings."""
    client = chromadb.PersistentClient(path="./chroma_db")
    model = SentenceTransformer('all-MiniLM-L6-v2')
    return client, model

def load_and_clean_othello():
    """Charge le texte et retire les headers."""
    if not os.path.exists("othello.txt"):
        return "Fichier othello.txt manquant."
...
def call_llm(query, context, api_key=None):
    """Appelle OpenAI ou LM Studio sans l'argument 'proxies'."""
    if api_key:
...
def main():
    ...
    if prompt := st.chat_input("Pose ta question sur Othello..."):
        ...
        with st.chat_message("assistant"):
            try:
                ...
                answer = f"Erreur de connexion. Vérifiez LM Studio ou votre clé. \n\nSources : {context}"
